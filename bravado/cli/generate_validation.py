...
def validate_schema_object(swagger_spec, schema_object_spec):
    """
    ...
    if obj_type in SWAGGER_PRIMITIVES:
        return validate_primitive(swagger_spec, schema_object_spec)

    elif obj_type == 'array':
        return validate_array(swagger_spec, schema_object_spec)

    elif is_object(swagger_spec, schema_object_spec):
        return validate_object(swagger_spec, schema_object_spec)

    elif obj_type == 'file':
        pass

    else:
        raise SwaggerMappingError('Unknown type {0} for schema {1}'.format(
            obj_type, schema_object_spec))


@scrub_sensitive_value
def validate_primitive(swagger_spec, primitive_spec):
    """
    ...
    get_validator_type(swagger_spec)(
        ...
        resolver=swagger_spec.resolver).validate()
